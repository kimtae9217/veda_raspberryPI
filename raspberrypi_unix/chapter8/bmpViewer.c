...
int main(int argc, char **argv)
{
    ...
    ubyte r, g, b, a = 255;
    ubyte *pData, *pBmpData, *pFbMap, *pAddData;
    ...
    /* 밝기 조절을 위한 새로운 버퍼 할당 */
    pAddData = (ubyte*)malloc(cols * rows * color/8);

    /* 밝기 조절 (+50) */
    for (int y = 0; y < rows; y++) {
        for (int x = 0; x < cols * color/8; x++) {
            int idx = y * cols * color/8 + x;
            pAddData[idx] = LIMIT_UBYTE(pData[idx] + 50);
        }
    } 
    ...
    for(int y = 0, k, total_y; y < rows; y++) {
        ...
        for(int x = 0; x < cols; x++) {
            ...
            b = LIMIT_UBYTE(pAddData[k+x*color/8+0]);
            g = LIMIT_UBYTE(pAddData[k+x*color/8+1]);
            r = LIMIT_UBYTE(pAddData[k+x*color/8+2]);
